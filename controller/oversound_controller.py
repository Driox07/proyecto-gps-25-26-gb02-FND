...
async def delete_user_account(request: Request, username: str):
    """
    Elimina la cuenta de usuario y todos sus datos asociados
    Si el usuario tiene un perfil de artista, lo elimina primero en TYA (con cascada)
    """
    token = request.cookies.get("oversound_auth")
    userdata = obtain_user_data(token)
    ...
    if userdata.get('username') != username:
        return JSONResponse(content={"error": "No autorizado para eliminar esta cuenta"}, status_code=403)
    
    try:
        # Si el usuario tiene un artistId, eliminar todos sus contenidos
        if userdata.get('artistId'):
            artist_id = userdata.get('artistId')
            
            try:
                # Obtener información del artista para saber qué eliminar
                artist_resp = requests.get(
                    f"{servers.TYA}/artist/{artist_id}",
                    timeout=5,
                    headers={"Accept": "application/json", "Cookie": f"oversound_auth={token}"}
                ...
                if artist_resp.ok:
                    ...
                    if artist_data.get('owner_songs'):
                        for song_id in artist_data.get('owner_songs', []):
                            try:
                                requests.delete(
                                    f"{servers.TYA}/song/{song_id}",
                                    timeout=5,
                                    headers={"Accept": "application/json", "Cookie": f"oversound_auth={token}"}
                                )
                            except Exception:
                    ...
                    if artist_data.get('owner_albums'):
                        for album_id in artist_data.get('owner_albums', []):
                            try:
                                requests.delete(
                    ...
                    # Eliminar todo el merchandising del artista
                    if artist_data.get('owner_merch'):
                        for merch_id in artist_data.get('owner_merch', []):
                            try:
                                requests.delete(
                                    f"{servers.TYA}/merch/{merch_id}",
                                    timeout=5,
                                    headers={"Accept": "application/json", "Cookie": f"oversound_auth={token}"}
                                )
                            except Exception:
        ...
        if user_delete_resp.ok:
            return JSONResponse(content={"message": "Cuenta eliminada exitosamente"}, status_code=200)
        else:
            error_data = user_delete_resp.json() if user_delete_resp.text else {}
            return JSONResponse(content=error_data, status_code=user_delete_resp.status_code)
            
    except requests.RequestException as e:
        print(f"[DELETE ACCOUNT] Error eliminando cuenta: {e}")
        try:
            error_data = e.response.json() if hasattr(e, 'response') else {}
            return JSONResponse(content=error_data, status_code=e.response.status_code if hasattr(e, 'response') else 500)
        except:
            return JSONResponse(content={"error": "Error al eliminar la cuenta"}, status_code=500)
